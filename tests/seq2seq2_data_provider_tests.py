...
batch_size = 3
sut = Seq2SeqDataProvider(data_reader, window_size=window_size, segment_size=segment_size,
...
def shape_test():
	for inputs, outputs in sut.next():
		...
		assert outputs.shape == (batch_size, segment_size), \
			f"outputs had the wrong shape. Expected: {(batch_size, segment_size)}, got: {outputs.shape}"
